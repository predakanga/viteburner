...
const path$d = require("node:path");
...
function createLogger() {
  const logger2 = {
    base: vite.createLogger("info", { prefix, allowClearScreen: false }),
...
class WatchManager {
  ...
  findItem(file2) {
    return this.items.find((item) => micromatch.isMatch(file2, item.pattern));
  }
  init() {
  ...
  triggerHmr(file2, event) {
    ...
    if (event !== "unlink" && fs$j.statSync(require$$1.resolve(root, file2)).mtimeMs <= this.enabledTimeStamp) {
      return;
    }
...
function handleKeyInput(wsAdapter) {
  ...
  const showRamUsageLocal = async () => {
    ...
    if (!fs$j.existsSync(require$$1.resolve(wsAdapter.server.config.root, file2))) {
...
class WsAdapter {
  ...
  async checkDependencies(data) {
    ...
    for (const item of data) {
      ...
      const resolvedFile = slash(require$$1.resolve(this.server.config.root, item.file));
      (_a = this.server._importGlobMap) == null ? void 0 : _a.forEach((value, key) => {
        if (value.some((pattern) => micromatch.match([resolvedFile], pattern).length > 0)) {
          const importer = slash(require$$1.relative(this.server.config.root, key));
  ...
  async fullDownload() {
    ...
    for (const [server, files] of filesMap) {
      ...
      for (const file2 of files) {
        ...
        const resolvedLocation = require$$1.resolve(this.server.config.root, location);
...
function getWss(port) {
  if (wss === null || wssPort !== port) {
    if (wss) {
      wss.clients.forEach((ws2) => ws2.close());
      wss.close();
    }
    wssPort = port;
...
const wsResponseSchema = zod.z.object({
  jsonrpc: zod.z.literal("2.0"),
  id: zod.z.number(),
  result: zod.z.any(),
  error: zod.z.any()
});
const pushFileResponseSchema = zod.z.literal("OK");
const getFileResponseSchema = zod.z.string();
const deleteFileResponseSchema = zod.z.literal("OK");
const getFileNamesResponseSchema = zod.z.array(zod.z.string());
const getAllFilesResponseSchema = zod.z.array(
  zod.z.object({
    filename: zod.z.string(),
    content: zod.z.string()
  })
);
const calculateRamResponseSchema = zod.z.number();
const getDefinitionFileResponseSchema = zod.z.string();
class WsManager {
  constructor(options) {
    __publicField(this, "options");
  ...
  get connected() {
    var _a;
    return ((_a = this.ws) == null ? void 0 : _a.readyState) === ws.WebSocket.OPEN;
  }
  _registerHandler() {
    const _onConnected = (ws2) => {
      this.ws = ws2;
      ws2.on("message", (response) => this.handleMessage(response));
    };
    const _onError = (e) => {
      const err = String(e);
  ...
  onConnected(cb) {
    const handler2 = async (ws2) => {
      this.ws = ws2;
      const unregister = await cb(ws2);
  ...
  async sendMessage(options) {
    ...
    const result = new Promise((resolve, reject) => {
      const onResolve = (data) => {
        var _a;
        try {
          resolve(((_a = options.validator) == null ? void 0 : _a.parse(data)) ?? data);
        } catch (e) {
          reject(e);
        } finally {
  ...
  return {
    ...
    configureServer(_server) {
      server = {
        ..._server,
        pathToId(file2) {
          const id = `/@fs/${slash(pathe.resolve(server.config.root, file2))}`;
...
const transformImports = (raw, externalValue, transformModuleName) => {
  var _a;
  const ast = acorn.Parser.parse(raw, {
    ecmaVersion: "latest",
    sourceType: "module"
  });
...
const _require = node_module.createRequire(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.src || new URL("index.js", document.baseURI).href);
...
async function loadBabel() {
  if (!babel) {
    babel = await Promise.resolve().then(() => require("./index-aee87c66.js")).then((n) => n.index);
  }
  return babel;
}
...
exports.calculateRamResponseSchema = calculateRamResponseSchema;
exports.commonjsGlobal = commonjsGlobal;
exports.createLogger = createLogger;
exports.defaultDownloadLocation = defaultDownloadLocation;
exports.defaultDts = defaultDts;
exports.defaultUploadLocation = defaultUploadLocation;
exports.defineConfig = defineConfig;
exports.deleteFileResponseSchema = deleteFileResponseSchema;
exports.displayKeyHelpHint = displayKeyHelpHint;
exports.displayWatchAndHelp = displayWatchAndHelp;
exports.externalRE = externalRE;
exports.fixImportPath = fixImportPath;
exports.fixStartingSlash = fixStartingSlash;
exports.forceStartingSlash = forceStartingSlash;
exports.formatDownload = formatDownload;
exports.formatError = formatError;
exports.formatNormal = formatNormal;
exports.formatUpload = formatUpload;
exports.formatWarn = formatWarn;
exports.getAllFilesResponseSchema = getAllFilesResponseSchema;
exports.getAugmentedNamespace = getAugmentedNamespace;
exports.getDefaultConfig = getDefaultConfig;
exports.getDefaultExportFromCjs = getDefaultExportFromCjs;
exports.getDefinitionFileResponseSchema = getDefinitionFileResponseSchema;
exports.getFileNamesResponseSchema = getFileNamesResponseSchema;
exports.getFileResponseSchema = getFileResponseSchema;
exports.getSourceMapString = getSourceMapString;
exports.getWss = getWss;
exports.handleKeyInput = handleKeyInput;
exports.hmrPluginName = hmrPluginName;
exports.isExternalUrl = isExternalUrl;
exports.isScriptFile = isScriptFile;
exports.isWssReused = isWssReused;
exports.loadConfig = loadConfig;
exports.logger = logger;
exports.main = main;
exports.onKeypress = onKeypress;
exports.prefix = prefix;
exports.pushFileResponseSchema = pushFileResponseSchema;
exports.removeStartingSlash = removeStartingSlash;
exports.resolveConfig = resolveConfig;
exports.resolveDts = resolveDts;
exports.resolveDumpFile = resolveDumpFile;
exports.resolvePolling = resolvePolling;
exports.resolveWatchLocation = resolveWatchLocation;
exports.setHandler = setHandler;
exports.startDev = startDev;
exports.virtualModuleId = virtualModuleId;
exports.viteburnerPlugin = viteburnerPlugin;
exports.writeFile = writeFile$1;
exports.wsResponseSchema = wsResponseSchema;
